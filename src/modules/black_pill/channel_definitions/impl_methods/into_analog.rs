...
#[macro_export]
macro_rules! into_analog {
    ( $pin: ident, $gpio_group: ident ) => {
        pub fn into_analog(self, crh: &mut stm32f1xx_hal::gpio::$gpio_group::CRH) -> Self {
            match self {
                Channel::$pin(mode) => {
                    match mode {
                        $pin::Mode::Input(input) => {
                            match input {
                                $pin::Input::Analog(p) => {
                                    Channel::$pin($pin::Mode::Input($pin::Input::Analog(p.into_analog(crh))))
                                }
